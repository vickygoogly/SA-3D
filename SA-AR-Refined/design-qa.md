...
## AR correction — v6.2 / 23 September 2026

The AR controller now uses a dedicated transparent DOM overlay and the actual shared operational scene. Placement is explicitly gated on a tap and a recent horizontal-plane hit. The dashboard is hidden/inert throughout AR, and asset panels cannot open before placement. Workers, vehicles, asset markers, alerts, IVMS, vitals, PTW and recorded feeds use the existing data/interaction paths. Exit and failure cleanup restore the desktop scene and controls.

API/model/road-clearance checks and ten new placement/session tests pass. The AR lifecycle tests use simulated WebXR and DOM APIs; they do not provide a browser layout or physical-device QA pass. The fresh local Chromium download returned invalid/truncated archives. Physical AR camera compositing, mobile performance, real-world tracking and video playback remain unverified. See `docs/AR-VALIDATION.md`. The historical visual-refinement record below is retained; no new visual fidelity pass is claimed.
