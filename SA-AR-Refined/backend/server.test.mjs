...
test('backend serves frontend/model, persists acknowledgment and resets demo',async()=>{
 ...
 try{
  ...
  for(const asset of ['/operations.css','/refinement.css','/operations.js','/ar.css','/ar-placement.js','/ar-experience.js','/vendor/lucide.min.js'])assert.equal((await fetch(base+asset)).status,200,asset+' must be served');
  ...
  const campus=await loadCampus(base+'/models/campus.glb');assert.ok(campus.children.length>800);const bounds=new THREE.Box3().setFromObject(campus).getSize(new THREE.Vector3());assert.ok(bounds.x>180&&bounds.y>30&&bounds.z>100,'Entire campus geometry and transforms must survive loading');
