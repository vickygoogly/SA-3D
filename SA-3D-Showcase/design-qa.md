...
- Restored `page-name` and `page-desc` controller targets required by the original route handler; this was the cause of the navigation failure in the previous package.
- Removed all rendered image-based 3D surfaces. Water is now an animated Three.js mesh with a physical material, and placeholder satellite islands are hidden.
