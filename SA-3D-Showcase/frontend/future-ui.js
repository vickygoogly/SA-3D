...
top.innerHTML=`<div class="atlas-mark"><span class="atlas-glyph">◆</span><strong>SIGNAL ATLAS</strong><span class="atlas-divider"></span><b>EPS-1</b><span>Industrial plant</span></div><div class="atlas-nav"><button class="atlas-nav-active" data-atlas-view="live">Live</button><button data-atlas-view="explore">Explore</button><button data-atlas-view="alerts">Events</button><button data-atlas-view="cameras">Reports</button></div><div class="atlas-operator"><span class="atlas-search">Search assets, zones, or events…</span><span class="atlas-user">OC</span><span><b>Operations controller</b><small>Control room</small></span></div>`;
...
const atlasRoutes={live:'overview',explore:'personnel',alerts:'alerts',cameras:'cameras'};
document.querySelectorAll('[data-atlas-view]').forEach(button=>button.addEventListener('click',()=>{const view=atlasRoutes[button.dataset.atlasView]||'overview';window.setView?.(view);document.querySelectorAll('[data-atlas-view]').forEach(item=>item.classList.toggle('atlas-nav-active',item===button));}));
// Keep the legacy IDs: app.js updates these nodes whenever a left or top route changes.
pageTitle.innerHTML=`<div class="atlas-title"><span class="eyebrow">EPS-1 · LIVE OPERATIONS</span><h1 id="page-name">Situational Awareness</h1><p id="page-desc">People, assets, and operational risk—under one spatial command view.</p></div><div class="atlas-date"><span>Operational status</span><strong><i></i> All systems online</strong><small id="atlas-clock">—</small></div>`;
