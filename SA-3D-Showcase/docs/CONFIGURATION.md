...
```json
{
  ...
  "cameraVideos": {
    "CAM-012": "/media/CCTV%201.mp4",
    "CAM-021": "/media/CCTV%202.mp4",
    "CAM-034": "/media/CCTV%203.mp4"
  },
  "droneVideos": {
    "DRN-001": "/media/drone%201.mp4",
    "DRN-002": "/media/drone%202.mp4",
    "DRN-003": "/media/drone%203.mp4"
  }
}
```
...
## Preloaded camera and drone footage

The supplied CCTV and drone videos are already mapped in `backend/config.json`; the user interface has no video-upload function. Click the relevant fixed-camera or drone marker on the 3D plant to open a clean looping feed. The feed removes browser playback duration controls so it presents as a live-view panel.
