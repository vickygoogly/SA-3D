...
const server=http.createServer(async (req,res)=>{
 try{
  ...
  if(route.startsWith('/api/')){
   ...
   if(req.method==='GET'&&route==='/api/config')return json(res,200,{modelUrl:config.modelUrl,cameraVideos:{...(config.cameraVideos||{}),...Object.fromEntries(Object.entries(uploadedVideos).map(([id,item])=>[id,item.url]))}});
