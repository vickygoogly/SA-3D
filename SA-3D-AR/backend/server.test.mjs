...
test('backend serves frontend/model, persists acknowledgment and resets demo',async()=>{
 ...
 try{
  ...
  for(const asset of ['/operations.css','/refinement.css','/operations.js','/ar.css','/ar-placement.js','/ar-experience.js','/plant-materials.js','/plant-environment.js','/personnel-renderer.js','/vendor/lucide.min.js'])assert.equal((await fetch(base+asset)).status,200,asset+' must be served');
  ...
  const campus=await loadCampus(base+'/models/campus.glb');assert.equal(campus.children.length,campus.userData.batches);assert.ok(campus.userData.sourceMeshes>5000);assert.ok(campus.children.length<campus.userData.sourceMeshes/4);assert.ok(campus.children.every(m=>m.material.name));const bounds=new THREE.Box3().setFromObject(campus).getSize(new THREE.Vector3());assert.ok(bounds.x>180&&bounds.y>30&&bounds.z>100,'Entire campus geometry and transforms must survive loading');
  ...
  for(const id of ['P-001','P-042','P-087','P-248']){const vitals=await (await fetch(base+'/api/personnel/'+id+'/vitals')).json();assert.equal(vitals.id,id);assert.equal(typeof vitals.heartRate,'number');}
