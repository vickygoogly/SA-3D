...
export function exportGLB(root,file){
 ...
 const doc={asset:{version:'2.0',generator:'TR AWARE plant exporter'},scene:0,scenes:[{name:root.name,extras:root.userData,nodes:[]}],nodes:[],meshes:[],materials:[],accessors:[],bufferViews:[],buffers:[{byteLength:0}],extensionsUsed:['KHR_materials_emissive_strength']};
 ...
 root.traverse(obj=>{
  ...
  if(material===undefined){material=doc.materials.push({name:m.name||'Plant material',extras:m.userData,pbrMetallicRoughness:{baseColorFactor:[...m.color.toArray(),m.opacity],metallicFactor:m.metalness??0,roughnessFactor:m.roughness??1},emissiveFactor:m.emissive?.toArray()||[0,0,0],extensions:{KHR_materials_emissive_strength:{emissiveStrength:m.emissiveIntensity??1}},doubleSided:m.side===2})-1;materials.set(m.uuid,material);}
