...
## Verification scope after v6.3

A working local Chromium/WebGL browser is now available and was used for the model rendering and interaction checks recorded in `browser-results.json`. The new detailed model and instanced personnel pass the geometric checks, including individual worker picking under AR transforms.

Physical camera compositing, real plane placement, tracking accuracy and phone/tablet performance remain unverified here. The WebXR tests use simulated session/input APIs; they do not replace the physical-device acceptance below.
...
2. Scan a table or clear floor. A wall must not enable placement. Detecting a surface without tapping must not show the plant. Losing the detected plane must remove the ring. Tap to place: the model appears once, with the base of its support structure resting on the surface and no details panel open.
