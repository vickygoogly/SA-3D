...
mats.deck.color.setHex(0x263a46); mats.deck.roughness=.7;
mats.steel.color.setHex(0xa4b5ba); mats.steel.metalness=.82; mats.steel.roughness=.28;
mats.pipe.color.setHex(0x5d91a0); mats.pipe.metalness=.78; mats.pipe.roughness=.32;
mats.yellow.color.setHex(0xc49a4c);
const lampMaterial=new T.MeshStandardMaterial({color:0xffe3ad,emissive:0xffd18b,emissiveIntensity:3});
for(let x=-84;x<=84;x+=14)for(const z of [-66,65]){pipe([x,0,z],[x,6,z],.07,'dark');box(x,6,z,1.1,.2,.5,lampMaterial);}
// Dedicated transverse service lanes connect the two existing longitudinal roads.
for(const z of [-58,57]){box(.5,.08,z,67,.14,10,'dark');for(let x=-28;x<31;x+=8)box(x,.18,z,3,.04,.15,'yellow');}
plant.name='EPS-1 Refined Original Plant';
...
exportGLB(plant,new URL('../models/campus.glb',import.meta.url));
