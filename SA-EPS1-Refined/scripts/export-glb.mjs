...
export function exportGLB(root,file){
 ...
 root.traverse(obj=>{
  ...
  if(material===undefined){material=doc.materials.push({name:m.name||'Plant material',pbrMetallicRoughness:{baseColorFactor:[...m.color.toArray(),m.opacity],metallicFactor:m.metalness??0,roughnessFactor:m.roughness??1},emissiveFactor:m.emissive?.toArray()||[0,0,0],doubleSided:m.side===2})-1;materials.set(m.uuid,material);}
