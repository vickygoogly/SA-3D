...
for(let i=0;i<5;i++){let z=-47+i*6;pipe([-75,2,z],[-43,2,z],.55);for(const x of [-69,-57,-46]){box(x,1,z,1.3,2,1.5,'dark');const wheel=mesh(new T.TorusGeometry(.7,.11,6,12),'yellow',x,3,z);wheel.rotation.y=Math.PI/2;}pipe([-75,2,z],[-75,4,-21],.42);}
...
function tower(x,z,h){for(const sx of [-1,1])for(const sz of [-1,1])pipe([x+sx*5,1,z+sz*5],[x+sx*1.6,h,z+sz*1.6],.28,'yellow');for(let y=3;y<h;y+=6){const w=5-3.4*y/h;for(const sz of [-1,1]){pipe([x-w,y,z+sz*w],[x+w,y+5,z+sz*(w-.45)],.13,'edge');pipe([x+w,y,z+sz*w],[x-w,y+5,z+sz*(w-.45)],.13,'edge');}box(x,y,z,w*2,.2,w*2,'dark')}}tower(80,44,38);
